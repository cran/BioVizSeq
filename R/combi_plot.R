...
combi_p <- function(tree_path, gff_path = NULL,
  ...
  if(is.null(gff_path)){
    ...
    p_gff <- gff_plot(gff_path, the_order = "the_order.csv") +
      labs(y="") + 
  ...
  if(is.null(meme_path)){
    ...
    p_meme <- meme_plot(meme_path, the_order = "the_order.csv") +
      labs(y="") + 
      theme(axis.title.y = element_blank(), 
            axis.text.y = element_blank(),
            axis.ticks.y = element_blank(), 
  ...
  if(!is.null(cdd_path) && !is.null(fa_path)){
    p_cdd <- cdd_plot(cdd_path, fa_path, the_order = "the_order.csv") +
      labs(y="") + 
      theme(axis.title.y = element_blank(), 
            axis.text.y = element_blank(),
            axis.ticks.y = element_blank(), 
  ...
  if(is.null(fa_path)){
    ...
    p_smart <- smart_plot(fa_path, the_order = "the_order.csv") +
      labs(y="") + 
      theme(axis.title.y = element_blank(), 
            axis.text.y = element_blank(),
            axis.ticks.y = element_blank(), 
  ...
  if(is.null(pfam_path)){
    ...
    p_pfam <- pfam_plot(pfam_path, the_order = "the_order.csv") +
      labs(y="") + 
      theme(axis.title.y = element_blank(), 
            axis.text.y = element_blank(),
            axis.ticks.y = element_blank(), 
  ...
  plot_list <- list(p_tree = p_tree, p_gff = p_gff, p_pfam = p_pfam, p_meme = p_meme, p_smart = p_smart, p_cdd = p_cdd )
